 ...
 *     
 *   The Geekduino has a 64 byte serial buffer, so messages of more than 64 characters will be cut off  
 *     
 *  Wiring
...
void loop()
{
  ...
  if (Serial.available()) {
    ...
    delay(100);
    // clear the screen
    lcd.clear(); //remove this line to fill the screen over multiple messages
    lcd.setCursor(0,0); //remove this line to fill the screen over multiple messages
